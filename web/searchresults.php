...
$singer=htmlspecialchars($_GET["search"]);
$singer     = mysqli_real_escape_string($mysqli,$singer);

if (empty($singer)) {
    $result = $mysqli->query("SELECT * FROM ng_singers");
}else{
    echo '<input  type="hidden" id="singersearch" value="'.$singer.'">';
    $result = $mysqli->query("SELECT * FROM ng_singers WHERE name LIKE '%$singer%'");
}
...
//now query for album details
$album=htmlspecialchars($_GET["album"]);
$album     = mysqli_real_escape_string($mysqli,$album);
$result2 = $mysqli->query("SELECT * FROM ng_albums WHERE album_name LIKE '%$album%'");
if (empty($album)) {
   
    $result2 = $mysqli->query("SELECT * FROM ng_albums");
}else{
    echo '<input type="hidden" id="albumsearch" value="'.$album.'">';
    $result2 = $mysqli->query("SELECT * FROM ng_albums WHERE album_name LIKE '%$album%'");
}
...
<header style="background-color: rgb(0,0,0); /* Fallback color */
  ...
  text-align: center;">
    ...
    <h3 id="searchgreeting"></h3>
    <div id="navButtons">
        <button class="button" onclick="displaySingers()">Singers</button>
        <button class="button" onclick="displayAlbums()">Albums</button>
    </div>
    <div class="search-container">
    
      
      <button style="background-color:red"><a style="color:white" href="home.php">Back to ALL songs</a></button>
